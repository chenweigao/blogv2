...
class MyPlugin extends obsidian.Plugin {
    ...
    onload() {
        ...
        this.addCommand({
            id: 'tag2link-text',
            name: '标签转为内链',
            callback: () => this.标签转为内链(),
            hotkeys: [{ modifiers: ["Mod","Alt","Shift"], key: "3" } ]
        });
        this.addCommand({
            id: 'link2tag-text',
            name: '内链转为标签',
            callback: () => this.内链转为标签()
        });
        ...
        this.registerEvent(this.app.workspace.on('file-open', (file) => {
            if (file && file.path) {
                ...
                if(this.settings.version != 当前版本){
                    ...
                    noticeContent.createEl("p").innerHTML = "📣 欢迎使用 增强编辑 " + 当前版本 + " 插件！";
                    noticeContent.createEl("p").innerHTML = 功能更新;
                    noticeContent.createEl("p").innerHTML = 宣传页面;
    ...
    实用命令菜单() {
        ...
        this.registerDomEvent(this.statusBarIcon, "click", (e) => {
            ...
            menu.addItem((item) => {
                item.setTitle("导出搜索结果");
                item.setIcon("link");
                item.onClick(() =>this.导出搜索结果());
            });
            menu.addItem((item) => {
    ...
    async loadSettings() {
        console.log('增强编辑 已完成加载，有问题请联系蚕子（qq:312815311）');
        ...
        if(所选文本 == ""){
            ...
            if(增加){
                if(/^#+ /.test(当前行文本)){
                    ...
                    let _str = 当前行文本.replace(/^(#{2,6}) .*$/,"$1");   //获取前面的多个#号
                    //new obsidian.Notice("当前为标题行 "+_str);
                    // 如果 前缀# >= 2 个，替换掉首个#符号，即提升一级
                    let 新标题 = 当前行文本.replace(/^#/,"")
                    笔记全文.replaceRange(新标题,{line:当前行号,ch:0},{line:当前行号,ch:当前行文本.length});
                    var 末行行号 = 编辑模式.lastLine();
                    //console.log(当前行号+" "+末行行号);
                    let 临时行文本
                    for (var i = 当前行号+1; i <= 末行行号; i++) {
                        临时行文本 = 编辑模式.getLine(i);
                        if(/^#+ /.test(临时行文本)){
                            //console.log(i+" "+临时行文本);
                            // 临时行文本包含当前标题的 前缀# 符号组，代表下级标题
                            if(eval("/^"+_str+"(?=#)/").test(临时行文本)){
                                let 提升标题 = 临时行文本.replace(/^#/,"")
                                笔记全文.replaceRange(提升标题,{line:i,ch:0},{line:i,ch:临时行文本.length});
                            }else{
                                break
                            }
                        }
                ...
                if(/^#+ /.test(当前行文本)){
                    this.app.commands.executeCommandById('editor:unfold-all');
                    let _str = 当前行文本.replace(/^(#{1,5}) .*$/,"$1");   //获取前面的多个#号
                    //new obsidian.Notice("当前为标题行 "+_str);
                    // 如果 前缀# >= 2 个，替换掉首个#符号，即提升一级
                    let 新标题 = 当前行文本.replace(/^/,"#")
                    笔记全文.replaceRange(新标题,{line:当前行号,ch:0},{line:当前行号,ch:当前行文本.length});
    ...
    */
    ...
    async 导出搜索结果(){
        ...
        let 当前文件名 = this.app.workspace.getActiveFile().basename; // + ".md";
        //console.log(当前文件名);
        if(/^搜索-[\d\w一-龟]+$/i.test(当前文件名)){    //当前笔记名称符合【搜索-关键词】才允许搜索
            let 关键词 = 当前文件名.replace(/^搜索-/,""); //提取笔记名称中的 关键词
            //console.log(关键词);
            for (const file of this.app.vault.getMarkdownFiles()) {
                if(file.basename != 当前文件名 && !/^搜索-/.test(file.basename)){
                    fileNameAry.push(file.basename);
                    let tempText = await this.获取文件内容(file);
                    if(tempText.includes(关键词)){    //如果文件内容包含关键词
                        搜索结果 += "↫***↫# 摘自 [["+file.basename+"]]↫↫";   //先显示摘文出处
                        
                        let tempAry = tempText.split("\n"); //将笔记内容按行拆分为数组
                        let btAry = []; //临时标题记录
                        let oldBtStr = ""; //历史标题前缀
                        let oldBtext = ""; //历史标题内容
                        for(var i =0; i < tempAry.length; i++){
                            let thisLine = tempAry[i];  //逐行分析
                            if(/^#+ .+$/.test(thisLine)){   //如果当前行 前缀为# ，视为标题行
                                let tmpBtStr = thisLine.replace(/^(#+) .+$/,"$1"); //提取临时标题前缀 #符号
                                let tmpBtext = thisLine.replace(/^#+ /,"");  //提取临时标题内容
                                if(oldBtStr.length>0){  //如果历史标题前缀有内容
                                    if(oldBtStr.length >= tmpBtStr.length){
                                        //如果当前行标题级别 等于 或 高于 历史标题级别，则清空临时标题记录
                                        btAry = [];
                                        oldBtStr = "";
                                        oldBtext = "";
                                    }
                                }
                                oldBtStr = tmpBtStr;    //视为发现下级标题，当前标题前缀归入历史
                                oldBtext = tmpBtext;
                                btAry.push(oldBtStr+" [["+file.basename+"#"+oldBtext+"|"+oldBtext+"]]")   //把标题的内链加入临时数组
                                //console.log(btAry.join("↫↫"));
                            }else{
                                let tmpLine = thisLine.replace(/[=\*#$`\[\]() ]+/g,"").replace(/\<[^\<\>]+\>/g,""); //清理行中的md语法或html语法
                                if(tmpLine.includes(关键词)){  //如果非标题行中包含关键词
                                    if(btAry.length>0){ //如果有临时标题记录
                                        搜索结果 += btAry.join("↫↫")+"↫↫";
                                        btAry = [];
                                        oldBtStr = "";
                                        oldBtext = "";
                                    ...
                                    //thisLine = thisLine.replace(reg,"==$1==").replace(/\<[^\<\>]+\>/g,"");
                                    搜索结果 += thisLine.replace(/\<[^\<\>]+\>/g,"")+"↫↫";
                                }
                            }
                        }
                    }else{
            ...
            this.app.workspace.getActiveViewOfType(obsidian.MarkdownView).editor.setValue(搜索结果);
            //new obsidian.Notice("📣 搜索完毕，结果已写入当前页面中...");
            this.app.internalPlugins.getPluginById('global-search').instance.openGlobalSearch("file:"+当前文件名+" /"+关键词);
        }else{
            new obsidian.Notice("📣 要求笔记名必须为【搜索-关键词】样式\n\n 方可执行【导出搜索结果】命令！");
        }
    }

    async 导出搜索结果old(){
        this.获取编辑器信息 ();
        if(所选文本 != "" && /^[\d\w一-龟]+$/i.test(所选文本)){
            let outext = "↫↫";
            let fileNameAry = [];
            let 当前文件名 = this.app.workspace.getActiveFile().basename + ".md";
            for (const file of this.app.vault.getMarkdownFiles()) {
                if(file.name != 当前文件名){
                    fileNameAry.push(file.basename);
                    let tempText = await this.获取文件内容(file);
                    if(tempText.includes(所选文本)){
                        //tempText = tempText.replace(/(\r\n|\n)/g,"↫");
                        //let reg1 = new RegExp(`[^↫]*${所选文本}[^↫]*`,'g');
                        let reg1 = new RegExp(`(^#+ .+$|^.*${所选文本}.*$)`,'mg');
                        let results = tempText.match(reg1);
                        outext += "***↫摘自 [["+file.basename+"]]↫↫"+results+"↫↫"
                        //fileTexts.push("***↫摘自 [["+file.basename+"]]↫↫"+results+"↫");
                        //console.log(file.name+"\n"+results);
                    }else{
                        continue;
                    }
            ...
            //outext = fileTexts.toString().replace(/,/g,"↫").replace(/↫/g,"\n");
            let reg2 = new RegExp(`(?<!==)(${所选文本})(?!==)`,'g');
            outext = outext.replace(/,/g,"↫↫").replace(/↫[ 　]*/g,"\n").replace(reg2,"==$1==");
            navigator.clipboard.writeText(outext);
            new obsidian.Notice("📣 已搜索完毕，请粘贴结果...");
        }else{
            new obsidian.Notice("📣 请划选不含空格的纯文本，谢谢配合！");
            return
        }
    }
    ...
    */
    ...
    全文首行缩进() {
        ...
        if(isIndent){
            笔记正文 = 笔记正文.replace(/^[‌‌‌‌　]+/mg,"");
            笔记正文 = 笔记正文.replace(/^(?!(\s*\d+\.\s|[a-zA-Z一-龟]+:|\s*\-\.？\s|[\n\s\>#]+|```|\-\-\-|\|[^\|]|\*\*\*))/mg,"‌‌‌‌　　");
            isIndent = false;
        }else{
            笔记正文 = 笔记正文.replace(/^[‌‌‌‌　]+/mg,"");
            isIndent = true;
    ...
    打开pp视频() {
        var mp4 = "T:\视频资料\海蒂和爷爷-国德双音轨.mp4";
        var videoTime = "01:20:00,000";
        var command = 'PotPlayerMini64.exe "E:\Ob思维空间\视频\Scapple宣传视频.mp4"' //这里是执行的DOS命令
        JsExecCmd();
    }

    JsExecCmd() {
        const { exec } = require('child_process');
        console.log(`尝试执行cmd命令`);
...
class editSettingsTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        */
        ...
        mdText.appendChild(document.createElement('br'));
        mdText.appendText('调高标题级别「待设置」：将当前行标题（及子标题）或所选范围的标题级别调高。建议：空（1977878681）');
        mdText.appendChild(document.createElement('br'));
        mdText.appendText('调低标题级别「待设置」：将当前行标题（及子标题）或所选范围的标题级别调低。建议：空（1977878681）');
        mdText.appendChild(document.createElement('br'));
        //mdText.appendText('调高所有标题级别「待设置」：将所有标题的级别调高一级（最高为一级）。');
        //mdText.appendChild(document.createElement('br'));
        //mdText.appendText('调低所有标题级别「待设置」：将所有标题的级别调低一级（最低为六级）。建议：空 QQ:1977878681');
        //mdText.appendChild(document.createElement('br'));
        mdText.appendText('转换标签语法「Alt+Shift+3」：将选文转为或去除 #标签 效果');
        mdText.appendChild(document.createElement('br'));
        mdText.appendText('标签转为链接「Ctrl+Alt+Shift+3」：将 [[笔记名]] 与 #笔记名 效果互转');
        mdText.appendChild(document.createElement('br'));
        ...
        */
        ...
        toolText.appendChild(document.createElement('br'));
        toolText.appendText('导出搜索结果「待设置」：根据鼠标划选到的关键词来对整个库进行搜索，并将搜索结果写进剪贴板');
        toolText.appendChild(document.createElement('br'));
        toolText.appendText('获取搜索结果「待设置」：当在侧边栏进行搜索时，执行此命令可获取搜索结果（仅有笔记标题列表）');
        toolText.appendChild(document.createElement('br'));
        ...
        lineText.appendChild(document.createElement('br'));
        lineText.appendText('全文缩进「待设置」：在全文的每个行首添加两个全角空格，产生缩进效果');
        lineText.appendChild(document.createElement('br'));
