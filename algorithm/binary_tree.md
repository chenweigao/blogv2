...
➡️➡️➡️ 2024年9月20日 二刷，需要注意的点：`if not node.left and not node.right:` 终止条件的判断，左右子节点都没有了返回 `depth`.
...
>  2024年9月20日 comments：这个例题用了 depth, 来判断是奇数层级还是偶数层级，也不复杂。
...
上面的代码是之前的 blog 解法，切忌死记硬背。

#### BST 特性解法

这道题目除了可以根据中序遍历来求解外，也可以根据 BST 的特点进行求解。

对于某个节点 node 的“下一个节点”，在二叉搜索树中，肯定是在其右子树中，并且是大于该节点的所有节点中值最小的一个。
...
如上图所示，如果要找到 7 的“下一个节点“ 8，就是其右节点；5 的下一个节点 6，就是其右边节点中最小的一个。

两种情况：
1. 如果 node 节点值大于 p 的节点值，则其前继节点可能是 node 或者在左子树中
2. 如果 node 的节点值小于等于 p 的节点值，则 p 的后继节点可能在 node 的右子树中

```python
class Solution:
    def inorderSuccessor(self, root: TreeNode, p: TreeNode) -> TreeNode:
        successor = None
        # 如果存在右子树，则肯定是在右子树中的最左侧节点
        if p.right:
            successor = p.right
            while successor:
                successor = successor.left
                return successor
        # 如果右子树为空，则需要遍历找到 p 的祖先节点，如图中 17
        node = root
        while node:
            if node.val > p.val:
                successor = node
